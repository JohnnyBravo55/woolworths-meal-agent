...
_NZ_BASE = (
    ...
    "PRACTICAL lunch timing: never use the same day's dinner for lunch — lunch happens first. "
    "Day 1 lunch must be a simple ORIGINAL lunch (own protein, carb, veg). "
    "From day 2 onward, practical lunches reuse the PREVIOUS day's dinner leftovers only "
    "(Monday dinner → Tuesday lunch, etc.). "
    "Match leftover form to lunch format: wraps/sandwiches only for firm leftovers "
    "(roast/grill meat, patties, salad-friendly strips). "
    "Never put curry, stew, soup, or other saucy/wet leftovers in wraps or sandwiches — "
    "reheat those with rice (cook extra rice at lunch or use leftover rice). "
    "Portion sizes: ingredients are already sized for the household's adult-equivalent "
    "servings (children under 13 count as fractional adults); cook about half again "
    "extra protein/carb at dinner only when that dinner feeds tomorrow's leftover lunch. "
    "Do not double pack/block staples (taco packs, cheese blocks, wrap packs) for leftovers. "
